    ...
    def __getitem__(self, item):
        if item < self.len:
            return tf.convert_to_tensor(self.code[item, :]), \
                   ...
                   tf.convert_to_tensor(self.code[(item+self.neg_index[item])%self.len, :]), \
...
def get_test_dataset(qurey=False,poolsize=poolsize):
    a=loader(test)
    if qurey:
        a.reas_qurey()
...
def get_ours_test_dataset():
    return tf.data.Dataset.from_generator( lambda : ours(test),(tf.int64,tf.int64,tf.int64,tf.int64,tf.int64,tf.int64,tf.int64)).batch(500)
