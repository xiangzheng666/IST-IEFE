...
for epoch in range(epoch_num):
    ...
    with tqdm(total=int(num+1)) as pbar:
        for code,api,fun_name,neg_code,neg_api,neg_fun_name,code_desc,qurey,desc in train_db:
            ...
            with tf.GradientTape() as tape:
                neg_repersent,code_repersent,code_desc_repersent=model([code,api,fun_name,neg_code,neg_api,neg_fun_name,code_desc,qurey,desc])
    ...
    if np.mean(test_mrr) > score:
        ...
        model.save_weights("../model/UNIF/"+language+"/UNIF.pth",overwrite=True)
