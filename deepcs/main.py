...
def simlity_loss(neg_code_desc_repersent, code_repersent, code_desc_repersent):
    neg_score=tf.keras.losses.cosine_similarity(code_repersent,neg_code_desc_repersent ,axis=1)  # [batchsize]
...
for epoch in range(epoch_num):
    ...
    with tqdm(total=int(num+1)) as pbar:
        for code,api,fun_name,neg_qurey,code_desc,qurey,desc in train_db:
            pbar.update(1)
            with tf.GradientTape() as tape:
                neg_code_desc_repersent,code_repersent,code_desc_repersent=model([code,api,fun_name,neg_qurey,code_desc,qurey,desc])
                epoch_loss=simlity_loss(neg_code_desc_repersent, code_repersent, code_desc_repersent)
    ...
    if np.mean(test_mrr) > score:
        ...
        model.save_weights("../model/deepcs/"+language+"/deepcs.pth",overwrite=True)
